// app/(root)/(website)/khalti-return/KhaltiReturnClient.jsx
"use client";
...
export default function KhaltiReturnClient() {
  ...
  const pidx = useMemo(() => search.get("pidx") || "", [search]);
  const purchaseOrderId = useMemo(
    () => search.get("purchase_order_id") || "",
    [search]
  );
  const statusFromQuery = useMemo(() => search.get("status") || "", [search]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled && json?.success && json?.order?.display_order_id) {
          dispatch(clearCart());
  ...
  }, [dispatch, router, pidx, purchaseOrderId, statusFromQuery]);
    ...
    <div className="max-w-xl mx-auto px-4 py-16 text-center">
      ...
      {state.rawStatus === "Pending" && (
        <p className="text-sm text-slate-600 mb-6">
          If this remains pending for more than a couple of minutes, please
