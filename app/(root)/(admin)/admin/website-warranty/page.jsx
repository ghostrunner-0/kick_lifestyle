...
/* --------- PRINTING --------- */
/* Exact structure you asked for, with safe width & auto-shrinking product line */
function printRow(rowData) {
  ...
  let warranty_class = rowData.warranty_period == 365 ? "111px" : "-60px";
...
/* Build a compact single-line summary for the order (used on every card) */
const summarizeProductsForPrint = (rowsArr) => {
  ...
  // Single type → keep variant and show *qty
  if (list.length === 1) {
    const only = list[0];
    return {
  ...
  // Mixed → top 1–2 names with quantities then +N more, omit variant to save space
  const first = list[0];
  const second = list[1];
...
/* --------- Combobox (dark-mode friendly + long-label safe) --------- */
const OrdersComboBox = forwardRef(function OrdersComboBox(
  ...
  const selected = options.find((o) => String(o.value) === String(value))?.label || "";
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          <span className={cn(selected ? "" : "text-muted-foreground", "block truncate text-left min-w-0")}>
      ...
      <PopoverContent className="w-[--radix-popover-trigger-width] p-0" align="start">
        <Command shouldFilter={false}>
          <CommandInput placeholder="Search…" value={filterText} onValueChange={setFilterText} />
          <CommandList>
            {loading ? (
              ...
              <>
                ...
                <CommandGroup>
                  {filtered.map((opt) => (
                    ...
                    >
                      <Check
                        className={cn("mr-2 h-4 w-4", String(value) === String(opt.value) ? "opacity-100" : "opacity-0")}
                      />
...
export default function WarrantyFromOrdersFast() {
  ...
  const loadReadyToPackOrders = useCallback(async () => {
    ...
    try {
      const { data } = await axios.get("/api/admin/orders", {
        ...
        params: { page: 1, limit: 300, statuses: ["ready to pack"], sort: "createdAt:desc" },
      ...
      setOrderOptions(
        items.map((o) => {
          ...
          return { value: id, label: `${disp} — ${name}${phone ? ` (${phone})` : ""} — ${total}` };
        })
      );
    } catch (e) {
      setOrderOptions([]);
      showToast("error", e?.response?.data?.message || e?.message || "Failed to load orders");
    } finally {
  ...
  const fetchWarrantyForProducts = useCallback(async (productIds = []) => {
    ...
    for (const pid of unique) {
      try {
        let months = null;
        try {
          const { data } = await axios.get(`/api/product/get/${pid}`, { withCredentials: true });
          months = toNum(data?.data?.warrantyMonths ?? data?.warrantyMonths, null);
        } catch {
          const { data } = await axios.get("/api/product", {
            withCredentials: true,
            params: { start: 0, size: 1, filters: "[]", globalFilter: String(pid), sorting: "[]", deleType: "SD" },
          });
      ...
      try {
        ...
        (doc.items || []).forEach((it, idx) => {
          ...
          for (let i = 0; i < qty; i++) {
            expanded.push({
              ...base,
              _key: `${productId}-${base.variantId || "novar"}-${idx}-${i}-${Date.now()}`,
              unitIndex: i + 1,
              serialNumber: "",
              warrantyMonths: null,
            });
        ...
        setRows((prev) => prev.map((r) => ({ ...r, warrantyMonths: map[r.productId] ?? 12 })));
  ...
  const setSNAtIndex = (rowIndex, sn) =>
    setRows((prev) => {
      const next = [...prev];
      if (next[rowIndex]) next[rowIndex] = { ...next[rowIndex], serialNumber: sn };
      return next;
    });

  const allFilled = (arr) => arr.every((r) => (r.serialNumber || "").trim().length > 0);
  ...
  /* PRINT: summarize once per order, print one card per row (serial) */
// --- replace the whole doPrint with this ---
const doPrint = async (rowsSnapshot) => {
  ...
  const customerStr = seq ? `${seq} | ${name}${phone ? ` (${phone})` : ""}` : `${name}${phone ? ` (${phone})` : ""}`;

  // 1) prepare payload for DB
  const itemsForDB = (rowsSnapshot || rows).map((r) => ({
    ...
    warrantyMonths: Number(warrantyByProduct[r.productId] ?? r.warrantyMonths ?? 12),
  }));
  ...
  // 2) save (non-blocking UX: we still print even if it errors)
  try {
    ...
    if (data?.success) {
      ...
      showToast("success", `Saved ${ok}${dup ? `, ${dup} duplicate` : ""}${fail ? `, ${fail} failed` : ""}`);
    } else {
      showToast("error", data?.message || "Failed to save warranty");
    }
  } catch (e) {
    // keep the scanner flow fast; we still print
    showToast("error", e?.response?.data?.message || e?.message || "Save failed");
  }

  // 3) print (one card per row/serial) using your compact order summary
  const summary = summarizeProductsForPrint(rowsSnapshot || rows);

  (rowsSnapshot || rows).forEach((r, i) => {
    const months = Number(warrantyByProduct[r.productId] ?? r.warrantyMonths ?? 12);
  ...
  const onSerialKeyDown = (e, idx) => {
    ...
    const now = Date.now();
    if (now - enterGuardRef.current < 220) return; // guard double CR/LF
    enterGuardRef.current = now;

    const currentValue = e.currentTarget.value.trim();

    setRows((prev) => {
      const next = prev.map((r, i) => (i === idx ? { ...r, serialNumber: currentValue } : r));
      if (allFilled(next)) {
  ...
  const onPasteMulti = (e, rowIndex) => {
    ...
    setRows((prev) => {
      ...
      for (let i = 0; i < parts.length && rowIndex + i < next.length; i++) {
        next[rowIndex + i] = { ...next[rowIndex + i], serialNumber: parts[i] };
      }
      if (allFilled(next)) {
        setTimeout(() => doPrintAndReset(next), 0);
      } else {
  ...
  const orderSummary = useMemo(() => {
    ...
    const totalQty = (order.items || []).reduce((a, b) => a + Number(b?.qty || 0), 0);
    return {
    ...
    <div className="p-3 sm:p-6 space-y-6">
      <div className="flex items-center gap-2">
        ...
        <div className="ml-auto">
          <Button variant="outline" size="icon" onClick={loadReadyToPackOrders} title="Refresh list">
            <RefreshCw className="h-4 w-4" />
          </Button>
        </div>
      </div>

      {/* ONE ROW: dropdown + live summary */}
      <Card>
            ...
            {orderSummary && (
              <div className="sm:flex-1 sm:mt-6 sm:ml-2 min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <div className="font-medium truncate">{orderSummary.disp}</div>
                  <Badge className="capitalize" variant="secondary">{orderSummary.status}</Badge>
                  <Badge variant="outline" className="uppercase">{orderSummary.method}</Badge>
                  <div className="text-sm text-muted-foreground truncate">
                    {orderSummary.name}{orderSummary.phone ? ` • ${orderSummary.phone}` : ""} • {orderSummary.totalQty} unit{orderSummary.totalQty === 1 ? "" : "s"}
                  </div>
                  <div className="ml-auto text-sm font-medium">{orderSummary.total}</div>
                </div>
              </div>
            )}
          </div>
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Scan serials. Scanner hits <kbd>Enter</kbd>. When all rows are filled, it prints and resets, then focuses the dropdown.
          </p>
        </CardHeader>
        <CardContent>
                  ...
                  rows.map((r, idx) => (
                    <TableRow key={r._key}>
                      ...
                      <TableCell className="text-right">
                        {Number(warrantyByProduct[r.productId] ?? r.warrantyMonths ?? 12)}
                      </TableCell>
                      <TableCell>
                        <Input
