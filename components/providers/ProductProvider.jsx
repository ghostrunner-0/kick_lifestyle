...
export function ProductProvider({ children }) {
  ...
  let url = activeKey ? `/api/website/products?category=${encodeURIComponent(activeKey)}` : null;
  ...
  const { data, isLoading, error, refetch } = useFetch("website-products", url, {
  ...
  const value = useMemo(
    () => ({ activeKey, setActiveKey, products: data ?? [], isLoading, error, refetchActive }),
