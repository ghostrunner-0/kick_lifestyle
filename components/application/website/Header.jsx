...
export default function Header() {
  ...
  // Colors flip with sticky state
  const textCls = isSticky ? "text-gray-900" : "text-white";
  const iconCls = isSticky ? "text-gray-900" : "text-white";
  const hoverUnderlineColor = "var(--primary)"; // unchanged

  return (
    <>
      {/* FIXED header overlays page (banner shows behind it) */}
      <header
        className={[
          "fixed inset-x-0 top-0 z-50 transition-colors",
          isSticky ? "bg-white/70 backdrop-blur border-b" : "bg-transparent",
        ].join(" ")}
      >
        {/* fluid side padding: min 1rem, grow to 6rem */}
        <div className="mx-auto max-w-[1600px] [padding-inline:clamp(1rem,5vw,6rem)] flex items-center justify-between lg:py-5 py-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Sheet>
              <SheetTrigger asChild className="lg:hidden">
                <Button
                  ...
                  className={iconCls + " hover:bg-transparent"}
                >
            ...
            <div className="hidden lg:block">
              <Link href={WEBSITE_HOME} className="block">
                <Image
                  src={isSticky ? LOGO_BLACK : LOGO_WHITE}
                  height={146}
                  width={383}
                  alt="KICK"
                  className="w-32"
                ...
                item.items ? (
                  <div key={item.label} className="relative group">
                    ...
                    >
                      ...
                      <ChevronDown
                        className={`h-4 w-4 transition-transform duration-500 group-hover:rotate-180 ${iconCls}`}
                      />
                    </button>

                    {/* Dropdown (kept white for readability) */}
          ...
          <div className="flex items-center gap-1">
            <Button
              variant="ghost"
              size="icon"
              className={`hidden lg:inline-flex hover:bg-transparent ${iconCls}`}
              aria-label="Search"
              title="Search"
            >
