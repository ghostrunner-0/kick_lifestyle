...
export const FULL_MENU = [
  ...
  // Products (admin only)
  {
    ...
    subMenu: [
      ...
      { title: "All Variants", url: ADMIN_PRODUCT_VARIANT_ALL, roles: ["admin"] },
    ],
  },
  ...
  // Customer Service (admin + sales)
  {
    ...
    roles: ["admin", "sales"],
    subMenu: [
      { title: "Offline Shops", url: ADMIN_OFFLINE_SHOPS, roles: ["admin", "sales"] },
      { title: "Add", url: ADMIN_add_customer_service, roles: ["admin", "sales"] },
...
export function getAdminSidebar(role = "admin") {
  ...
  const filterByRole = (items) =>
    (items || [])
      .map((item) => {
        const children = Array.isArray(item.subMenu) ? filterByRole(item.subMenu) : undefined;
