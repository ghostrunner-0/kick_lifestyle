...
export const zSchema = z
  .object({
    ...
    warrantyMonths: z.coerce.number().min(0, "Warranty cannot be negative").optional(),
    ...
    // Product fields
    shortDesc: z.string().trim().max(300, "Short description must be at most 300 characters").optional(),
    category: objectId, // ref: "Category"
    mrp: z.coerce.number().positive("MRP must be > 0"),
    specialPrice: z.coerce.number().positive().optional(),

    productMedia: z.array(imageObject).min(1, "At least one product media is required"),
    descImages: z.array(imageObject).min(1, "At least one desc image is required"),
    heroImage: imageObject,
    additionalInfo: z.array(additionalInfoRow).min(1, "At least one additional info entry is required"),
